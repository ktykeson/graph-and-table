...
function App() {
    ...
    const generateTableDataForLine = (line) => {
        ...
        for (let x = -graphRange; pointsFound < 7 && x <= graphRange; x++) {
            ...
            if (Math.round(y) === y) { // Check if y is an integer
                tableData.push({ x, y });
                pointsFound++;
            }
        }
    ...
    const handleUserInput = (tableIndex, valueIndex, value) => {
        ...
        if (!updatedAnswers[tableIndex]) {
            updatedAnswers[tableIndex] = [];
        }
        updatedAnswers[tableIndex][valueIndex] = parseInt(value, 10);
    ...
    const checkAnswers = () => {
        return tablesData.every((table, tableIndex) => {
            return table.data.every((point, pointIndex) => {
                return userAnswers[tableIndex] && userAnswers[tableIndex][pointIndex] === point.y;
            });
        });
    };
    ...
    const handleMakeTables = () => {
        console.log('handleMakeTables called');
        const newTablesData = [];
    ...
    const renderTable = (tableData, index) => {
        ...
        return (
            <div key={`table-${index}`} className="table-container"> {/* Ensure key is unique and consistent */}
                            ...
                            exerciseMode ? (
                                <td key={idx}>
                                    <input
                                        type="number"
                                        defaultValue={y}
                                        onChange={(e) => handleUserInput(index, idx, e.target.value)}
            ...
            <div className="graph_box">
                ...
                <div style={{ width: "20%" }}>
                    ...
                    <div className="arrow_box">
                        ...
                        <br />
                        <button onClick={handleMakeTables} type="button" style={{marginTop: "10px"}}>
                            Make Tables
                        </button>
                        <button onClick={handleExerciseModeToggle} type="button" style={{marginTop: "10px"}}>
